...
grep -vF -f $local/comps_devset.txt temptrain.flist | grep 'comp-c\|comp-d' | sort >train_t.flist
grep -vF -f $local/comps_devset.txt temptrain.flist | grep -v 'comp-c\|comp-d' | sort >train_s.flist
grep -F -f $local/comps_devset.txt tempdecode.flist | grep 'comp-c\|comp-d' | sort >dev_t.flist
grep -F -f $local/comps_devset.txt tempdecode.flist | grep -v 'comp-c\|comp-d' | sort >dev_s.flist

# create utt2spk, spk2utt, txt, segments, scp, spk2gender
for x in train_s train_t dev_t dev_s; do
