...
class STIProcessor(DataProcessor):
    ...
    def add_examples_from_csv(
        ...
        data = (
            pd.read_csv(target_file, index_col="uttid")
            .loc[np.loadtxt(uttids, dtype='str')]
            .astype(np.int64)
        )

        labels = Patience.labels
        targets = data.values.tolist().copy()
        ids = data.index
