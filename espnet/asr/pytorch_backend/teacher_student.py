...
from espnet.asr.pytorch_backend.asr import CustomEvaluator, CustomUpdater, CustomConverter
...
def train(args):
    ...
    logging.warn(f"Training on {device}")
    ...
    trainer.extend(
        extensions.PlotReport(
            [
                ...
                "validation/main/loss_att",
                "main/loss_mlm",
                "validation/main/loss_mlm",
                "main/loss_ce",
                "validation/main/loss_ce",
                "main/loss_mse",
                "validation/main/loss_mse",
                "main/loss_cos",
                "validation/main/loss_cos",
            ]
            + ([] if args.num_encs == 1 else report_keys_loss_ctc),
            "epoch",
            file_name="loss_att.png",
        )
    )
    trainer.extend(
        extensions.PlotReport(
    ...
    if args.alpha_mse > 0:
        report_keys.insert(report_keys.index("main/loss_ce"), "main/loss_mse")
        report_keys.insert(report_keys.index("validation/main/loss_ce"), "validation/main/loss_mse")
    if args.alpha_cos > 0:
        report_keys.insert(report_keys.index("main/loss_ce"), "main/loss_cos")
        report_keys.insert(report_keys.index("validation/main/loss_ce"), "validation/main/loss_cos")
    if args.opt == "adadelta":
        trainer.extend(
