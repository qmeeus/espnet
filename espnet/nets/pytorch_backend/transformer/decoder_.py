...
class Decoder(ScorerInterface, torch.nn.Module):
    ...
    def __init__(self, odim,
        ...
        if input_layer == "embed":
            ...
            # TODO: input_layer's output dim should be the same as attention_dim? 
            # TODO: Add linear layer between input and positional encoding
            if emb_dim != attention_dim:
                self.embed = torch.nn.Sequential(
                    input_layer,
                    torch.nn.Linear(emb_dim, attention_dim),
                    pos_enc_class(attention_dim, positional_dropout_rate)
                )
            else:
                self.embed = torch.nn.Sequential(
        ...
        if attention_dim != emb_dim:
            self.output_layer = torch.nn.Linear(attention_dim, emb_dim)
        else:
