...
class Decoder(nn.Module, ScorerInterface):
    ...
    # def rnn_forward(self, ey, hidden_states):
    ...
    #         for layer in range(self.num_layers):
    #             hidden_states[layer], cell_states[layer] = self.rnn[layer](
    #                 ey if layer == 0 else hidden_states[layer - 1],
    ...
    #         for layer in range(self.num_layers):
    #             hidden_states[layer] = self.rnn[layer](
    #                 ey if layer == 0 else hidden_states[layer - 1],
    #                 hidden_states[layer]
    #             )
    ...
    def compute_loss(self, hs_pad, hlens, ys_pad, strm_idx=0):
        ...
        y_true = [torch.cat([y, eos], dim=0) for y in ys]
        y_true_pad = pad_list(y_true, self.ignore_id)
        ...
        acc = th_accuracy(predictions, y_true_pad, ignore_label=self.ignore_id)
        logging.debug('att loss:' + ''.join(str(loss.item()).split('\n')))

        # show predicted character sequence for debug
        if self.verbose > 1 and self.char_list is not None:
            self._print_predictions(predictions, y_true_pad)
    ...
    def _print_predictions(self, predictions, target):
        batch_size, max_output_length = target.size()
        ys_hat = predictions.view(batch_size, max_output_length, -1)
    ...
    def _recognize_beam(self, h, lpz, recog_args, char_list, rnnlm=None, strm_idx=0):
    ...
    def _recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, rnnlm=None,
        ...
        # initialization
        c_prev = [to_device(self, torch.zeros(n_bb, self.dunits)) for _ in range(self.num_layers)]
        z_prev = [to_device(self, torch.zeros(n_bb, self.dunits)) for _ in range(self.num_layers)]
        c_list = [to_device(self, torch.zeros(n_bb, self.dunits)) for _ in range(self.num_layers)]
        z_list = [to_device(self, torch.zeros(n_bb, self.dunits)) for _ in range(self.num_layers)]
        ...
        for i in six.moves.range(maxlen):
            ...
            z_prev = [torch.index_select(z_list[li].view(n_bb, -1), 0, vidx) for li in range(self.num_layers)]
            c_prev = [torch.index_select(c_list[li].view(n_bb, -1), 0, vidx) for li in range(self.num_layers)]
    ...
    def calculate_all_attentions(self, hs_pad, hlens, ys_pad, strm_idx=0, lang_ids=None):
