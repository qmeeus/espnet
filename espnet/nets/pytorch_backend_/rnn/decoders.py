...
class Decoder(torch.nn.Module, ScorerInterface):
    ...
    def __init__(self, eprojs, odim, dtype, dlayers, dunits, sos, eos, attention, verbose=0,
    ...
    def forward(self, hs_pad, hlens, ys_pad, strm_idx=0, lang_ids=None):
        ...
        if self.num_encs == 1:
            ...
            for idx in range(self.num_encs + 1):
                self.attention[idx].reset()  # reset pre-computation of h in atts and han
        ...
        for i in range(output_length):
            if self.num_encs == 1:
                att_c, att_w = self.attention[att_idx](hs_pad[0], hlens[0], self.dropout_dec[0](z_list[0]), att_w)
            else:
                for idx in range(self.num_encs):
                    att_c_list[idx], att_w_list[idx] = self.attention[idx](hs_pad[idx], hlens[idx],
                                                                     self.dropout_dec[0](z_list[0]), att_w_list[idx])
                hs_pad_han = torch.stack(att_c_list, dim=1)
                hlens_han = [self.num_encs] * len(ys_in)
                att_c, att_w_list[self.num_encs] = self.attention[self.num_encs](hs_pad_han, hlens_han,
    ...
    def compute_loss(self, outputs, target, strm_idx=0, lang_ids=None):
        batch_size, output_length = target.size()
        output_length += 1

        # FIXME
        ys = [y[y != self.ignore_id] for y in target]  # parse padded ys
        ylens = (target != self.ignore_id).sum(-1)
        ...
        loss = F.cross_entropy(outputs, ys_out_pad.view(-1),
        ...
        if self.labeldist is not None:
            if self.vlabeldist is None:
                self.vlabeldist = torch.from_numpy(self.labeldist).type_as(outputs)
            loss_reg = - torch.sum((F.log_softmax(outputs, dim=1) * self.vlabeldist).view(-1), dim=0) / batch_size
            loss = (1. - self.lsm_weight) * loss + self.lsm_weight * loss_reg

        return loss

    def compute_metrics(self, outputs, target, loss):
        batch_size, output_length = target.size()
        output_length += 1

        ys_out_pad = pad_list(target, self.ignore_id)
        accuracy = th_accuracy(outputs, ys_out_pad, ignore_label=self.ignore_id)
        perplexity = math.exp(loss.item())

        # show predicted character sequence for debug --> TODO: move
        if self.verbose > 1 and self.char_list is not None:
            ys_hat = outputs.view(batch_size, output_length, -1)
            ys_true = ys_out_pad
    ...
    def recognize_beam(self, h, lpz, recog_args, char_list, rnnlm=None, strm_idx=0):
        ...
        attention_idx = min(strm_idx, len(self.attention) - 1)
        # initialization
        ...
        for i in six.moves.range(maxlen):
            ...
            for hyp in hyps:
                ...
                if self.num_encs == 1:
                    att_c, att_w = self.attention[att_idx](h[0].unsqueeze(0), [h[0].size(0)],
                                                     self.dropout_dec[0](hyp['z_prev'][0]), hyp['a_prev'])
                else:
                    for idx in range(self.num_encs):
                        att_c_list[idx], att_w_list[idx] = self.attention[idx](h[idx].unsqueeze(0), [h[idx].size(0)],
                                                                         self.dropout_dec[0](hyp['z_prev'][0]),
                                                                         hyp['a_prev'][idx])
                    h_han = torch.stack(att_c_list, dim=1)
                    att_c, att_w_list[self.num_encs] = self.attention[self.num_encs](h_han, [self.num_encs],
                                                                               self.dropout_dec[0](hyp['z_prev'][0]),
    ...
    def recognize_beam_batch(self, h, hlens, lpz, recog_args, char_list, rnnlm=None,
        ...
        n_bb = batch * beam
        pad_b = (torch.arange(batch).type_as(h).long() * beam).view(-1, 1)
        ...
        # initialization
        vscores = torch.zeros(batch, beam)
        c_prev, z_prev, c_list, z_list = [[
            torch.zeros(n_bb, self.dunits) for _ in range(self.dlayers)
        ]] * 4
        

        rnnlm_state = None
        if self.num_encs == 1:
            a_prev, att_w_list, ctc_scorer, ctc_state = ([None],) * 4
            self.attention[att_idx].reset()  # reset pre-computation of h
        else:
        ...
        for i in six.moves.range(maxlen):
            ...
            vy = torch.LongTensor(self._get_last_yseq(yseq))
            ey = self.dropout_emb(self.embed(vy))
            if self.num_encs == 1:
                att_c, att_w = self.attention[att_idx](exp_h[0], exp_hlens[0], self.dropout_dec[0](z_prev[0]), a_prev[0])
                att_w_list = [att_w]
            else:
                for idx in range(self.num_encs):
                    att_c_list[idx], att_w_list[idx] = self.attention[idx](exp_h[idx], exp_hlens[idx],
                                                                     self.dropout_dec[0](z_prev[0]), a_prev[idx])
                exp_h_han = torch.stack(att_c_list, dim=1)
                att_c, att_w_list[self.num_encs] = self.attention[self.num_encs](exp_h_han, [self.num_encs] * n_bb,
            ...
            vidx = torch.LongTensor(accum_padded_beam_ids)
    ...
    def calculate_all_attentions(self, hs_pad, hlen, ys_pad, strm_idx=0, lang_ids=None):
        ...
        for i in six.moves.range(olength):
            if self.num_encs == 1:
                att_c, att_w = self.attention[att_idx](hs_pad[0], hlen[0], self.dropout_dec[0](z_list[0]), att_w)
                att_ws.append(att_w)
            else:
                for idx in range(self.num_encs):
                    att_c_list[idx], att_w_list[idx] = self.attention[idx](hs_pad[idx], hlen[idx],
                                                                     self.dropout_dec[0](z_list[0]), att_w_list[idx])
                hs_pad_han = torch.stack(att_c_list, dim=1)
                hlen_han = [self.num_encs] * len(ys_in)
                att_c, att_w_list[self.num_encs] = self.attention[self.num_encs](hs_pad_han, hlen_han,
                                                                           self.dropout_dec[0](z_list[0]),
                                                                           att_w_list[self.num_encs])
        ...
        if self.num_encs == 1:
            ...
            att_ws = att_to_numpy(att_ws, self.attention[att_idx])
        else:
            _att_ws = []
            for idx, ws in enumerate(zip(*att_ws)):
                ws = att_to_numpy(ws, self.attention[idx])
                _att_ws.append(ws)
            att_ws = _att_ws
        return att_ws
    ...
    def init_state(self, x):
        ...
        strm_index = 0
        att_idx = min(strm_index, len(self.attention) - 1)
        if self.num_encs == 1:
            a = None
            self.attention[att_idx].reset()  # reset pre-computation of h
        else:
